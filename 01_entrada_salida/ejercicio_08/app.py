...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        numOperadorA = int(txtOperadorA)
        txtOperadorB = self.txt_operador_b.get()
        numOperadorB = int(txtOperadorB)
        divi = numOperadorA % numOperadorB
        resto = 'El resto de dividir {0} por {1} es: {2}'.format(numOperadorA, numOperadorB, divi)
        alert(title='Resto de una division', message = resto)
